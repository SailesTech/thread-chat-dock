...
import { UnifiedSearch } from "@/components/sidebar/UnifiedSearch";
import { AttributeValueSelector } from "@/components/sidebar/AttributeValueSelector";
import { useNotionDatabases, useNotionAttributes } from "@/hooks/useNotionData";
...
export function AppSidebar() {
  ...
  // Funkcja do pobierania opcji dla select atrybutów z backendu
  const getAttributeOptions = (attribute: any) => {
    if (attribute.type === 'select' || attribute.type === 'multi_select') {
      // Tu powinna być logika pobierania opcji z Notion API
      // Na razie zwracam przykładowe opcje
      return [
                      ...
                      {selectedAttributes.includes(attr.id) && 
                       (attr.type === 'select' || attr.type === 'multi_select') && (
                        <AttributeValueSelector
                          ...
                          options={getAttributeOptions(attr)}
                        />
                      )}
                    </div>
                  ))}
